...
import com.pachain.android.adapter.viewresults.VotedResultRecycleAdapter;
import com.pachain.android.adapter.viewresults.VotedVoterRecycleAdapter;
...
public class PAChainViewVotingProgressActivity extends Activity implements View.OnClickListener, DatePicker.OnDateChangedListener {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        votedKeys = dataToolPackage.getVotedKeys(ballot.getElection());
        ...
        countiesMap = constantDataToolPackage.getCountiesMap();
    ...
    private void getVotedVoters(String state, String county, String precinct) {
        ...
        try {
            ...
            object.put("start", ToolPackage.ConvertToCommonStringByDate(tv_start.getText().toString().replace(getResources().getString(getResources().getIdentifier("viewVotingProgress_dateStart", "string", getPackageName())), "")));
            object.put("end", ToolPackage.ConvertToCommonStringByDate(tv_end.getText().toString().replace(getResources().getString(getResources().getIdentifier("viewVotingProgress_dateTo", "string", getPackageName())), "")));
            object.put("limit", pageRowCount);
            object.put("offset", pageRowCount * page);
            param = ecKeyUtil.encryptByPublicKey(object.toString(), ecKeyUtil.getPublicKeyFromString(SPUtils.getString(PAChainViewVotingProgressActivity.this, SPUtils.ENCRYPT_KEY, "")));

            params.add("params=" + URLEncoder.encode(param));
        } catch (Exception e) {
            e.printStackTrace();
        }
        PostApi api = new PostApi(Config.QUERY_VOTEDVOTERS, params);
            ...
            public void onSuccessed(String successed) {
                try {
                    ...
                    if (json.getBoolean("ret")) {
                        ...
                        Type type1 = new TypeToken<ArrayList<VotedVoterEntity>>() {}.getType();
                        votedVoters.addAll(gson.fromJson(response.getString("data"), type1));
                        for (VotedVoterEntity entity : votedVoters) {
                            ...
                            entity.setVotedCount(ToolPackage.decimalFormat(Integer.parseInt(entity.getVotedCount())));
                            entity.setVotingDate(ToolPackage.ConvertToSimpleStringByTime(entity.getVotingDate()));
                        }
                        if (votedVoters != null && votedVoters.size() > 0) {
                            if (page == 0) {
                                VotedVoterEntity entity = new VotedVoterEntity(getResources().getString(getResources().getIdentifier("viewVotingProgress_state", "string", getPackageName())),
                                    getResources().getString(getResources().getIdentifier("viewVotingProgress_county", "string", getPackageName())),
                                    getResources().getString(getResources().getIdentifier("viewVotingProgress_precinct", "string", getPackageName())),
                                    getResources().getString(getResources().getIdentifier("viewVotingProgress_count", "string", getPackageName())),
                                    getResources().getString(getResources().getIdentifier("viewVotingProgress_date", "string", getPackageName())));
                                votedVoters.add(0, entity);

                                if (dataArray.length() >= pageRowCount) {
    ...
    private void getVoterResults(String state, String county, String precinct, int seatID) {
        ...
        PostApi api = new PostApi(Config.QUERY_VOTERRESULTS, params);
        api.setOnApiListener(new PostApi.onApiListener() {
            @Override
            public void onExecute(String content) { }
            ...
            public void onSuccessed(String successed) {
                try {
                    ...
                    if (json.getBoolean("ret")) {
                        ...
                        Type type1 = new TypeToken<ArrayList<VotedResultEntity>>() {}.getType();
                        votedResults.addAll(gson.fromJson(response.getString("data"), type1));
                        for (VotedResultEntity entity : votedResults) {
                            entity.setVotingDate(ToolPackage.ConvertToSimpleStringByTime(entity.getVotingDate()));
                            entity.setSelected(votedKeys.containsKey(entity.getKey()) ? true : false);
                        }
                        if (votedResults != null && votedResults.size() > 0) {
                            if (page == 0) {
                                VotedResultEntity entity = new VotedResultEntity(getResources().getString(getResources().getIdentifier("viewVotingProgress_voter", "string", getPackageName())),
                                    getResources().getString(getResources().getIdentifier("viewVotingProgress_verificationCode", "string", getPackageName())),
                                    getResources().getString(getResources().getIdentifier("viewVotingProgress_votingResult", "string", getPackageName())),
                                    getResources().getString(getResources().getIdentifier("viewVotingProgress_date", "string", getPackageName())),
                                    false
                                );
    ...
    private void getSeats() {
        ...
        PostApi api = new PostApi(Config.GET_SEATS, params);
        api.setOnApiListener(new PostApi.onApiListener() {
            @Override
            public void onExecute(String content) { }
