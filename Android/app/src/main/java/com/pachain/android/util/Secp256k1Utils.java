...
public class Secp256k1Utils {
    ...
    static {
        Security.insertProviderAt(new org.spongycastle.jce.provider.BouncyCastleProvider(), 10);
    }

    public Secp256k1Utils(Context context) {
    ...
    public Map<String, Object> getKeyPair() {
        ...
        try {
            File privateKeyFile = context.getFileStreamPath(privateKeyFileName);
            File publicKeyFile = context.getFileStreamPath(publicKeyFileName);

            if (privateKeyFile.exists()) {
                String privateStr = readFromFile(privateKeyFile);
                String publicStr = readFromFile(publicKeyFile);
                ...
                savePrivateKey(Base64.encodeToString(((PrivateKey) ecKey.get("privateKey")).getEncoded(), Base64.NO_WRAP));
                savePublicKey(Base64.encodeToString(((PublicKey) ecKey.get("publicKey")).getEncoded(), Base64.NO_WRAP));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void savePrivateKey(String privateKeyStr) {
        FileOutputStream fos = null;
        try {
            fos = context.openFileOutput(privateKeyFileName, Context.MODE_PRIVATE);
    ...
    public void savePublicKey(String publicKeyStr) {
        FileOutputStream fos = null;
        try {
            fos = context.openFileOutput(publicKeyFileName, Context.MODE_PRIVATE);
    ...
    public Map<String, Object> generateSECP256K1Keypair() throws Exception {
        ...
        SPUtils.put(context, SPUtils.PREFS_KEY_REMIND_BACKUP, true);
        return keyPairMap;
    }
    ...
    public PublicKey createSharedPubKey(PrivateKey privateKey) throws GeneralSecurityException {
