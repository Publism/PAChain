package com.pachain.android.adapter.voting;
...
public class BallotHomeAdapter extends BaseAdapter {
    ...
    public View getView(final int i, View convertView, ViewGroup viewGroup) {
        ...
        if (model.getId() < 1 && (model.getSeat() == null || model.getSeat().getId() < 1) && model.getElection() != null && model.getElection().getId() > 0) {
            ...
            holder.tv_electionDay.setText(model.getElection().getDate());
            holder.tv_electionName.setText(model.getElection().getName());
            if (model.isVoting()) {
        ...
        } else if (model.getId() < 1 && model.getSeat() != null && model.getSeat().getId() > 0) {
            holder.ll_election.setVisibility(View.GONE);
            holder.ll_verifyTip.setVisibility(View.GONE);
            holder.ll_seat.setVisibility(View.VISIBLE);
            holder.ll_candidate.setVisibility(View.GONE);
            holder.tv_seatName.setText(model.getSeat().getName());
            ...
            holder.ch_check.setOnClickListener(new View.OnClickListener() {
               ...
               public void onClick(View v) {
                   ...
                   if (model.isVoting() && model.isVoted()) {
                       ...
                       int i = 0;
                       for (CandidateEntity candidateEntity : mData) {
                           if (candidateEntity.getId() > 0 && candidateEntity.getSeat().getId() == model.getSeat().getId() &&
